...
- OOV kelimeler için `float('nan')` döndürür (istisna değil — çağıranlar `math.isnan()` ile filtreleyebilir)
...
- `k > len(geçerli_kelimeler)` durumuna karşı koruma
...
| `word_similarity()` | Tamamlandı | sklearn ile kosinüs, OOV için NaN |
| `cluster_words()` | Tamamlandı | L2 normalizasyonu, random_state=42, n_init=10 |
